...
class AbstractProblem(ABC):
    ...
    def get_transitions(self, state, action):
        """
        Returns the probabilities for the next states and rewards when executing "action" from
        "state".
        :param state: a state of the problem.
        :param action: a valid action.
        :return: a list of tuples. Each tuple contains three elements:
        (probability, next state, reward), where probability is
        p(next state, reward | state, action).
        """
        pass
