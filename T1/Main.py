...
from PlotResults import plot_average_reward, plot_optimal_action_percentage
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run bandit experiments with optional parts.')
    parser.add_argument(
        '-a',
        '--no-parte-a',
        action='store_false',
    ...
    if args.parte_a:
        ...
        for agent_class, agent_name, agent_kwargs in agents_to_run:
            ...
            name, avg_rewards, avg_optimal_actions = testbed(
                agent_class, agent_name, **agent_kwargs
            )
    ...
    if args.parte_f:
        ...
        for agent_class, agent_name, agent_kwargs in agents_to_run:
            print(f'Running {agent_name}...')
            name, avg_rewards, avg_optimal_actions = testbed(
                agent_class, agent_name, **agent_kwargs, env_mean=0
            )
            results_dict[name] = (avg_rewards, avg_optimal_actions)
